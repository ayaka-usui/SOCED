...
function recurringA1(n1::Int64,n2::Int64,n3::Int64,n4::Int64,m1::Int64,m2::Int64,m3::Int64,m4::Int64)
    ...
    if m1 == m2 == m3 == 0
       coeff = (-1)*(n4-2m4+2)*(n4-2m4+1)/m4*recurringA0(M,1,1,1,m4)
    elseif m1 == m2 == 0
       coeff = (-1)*(n3-2m3+2)*(n3-2m3+1)/m3*recurringA0(M,1,1,m3,m4)
    elseif m1 == 0
       coeff = (-1)*(n2-2m2+2)*(n2-2m2+1)/m2*recurringA0(M,1,m2,m3,m4)
    else
       coeff = (-1)*(n1-2m1+2)*(n1-2m1+1)/m1*recurringA0(M,m1,m2,m3,m4)
    end
...
function Vijkl(n1::Int64,n2::Int64,n3::Int64,n4::Int64)
    ...
    # m1=m2=m3=m4=1
    matA[1,1,1,1] = 1.0 # exp(-2M*log(2)+sum(log.(M+1:2*M))-1/2*(sum(log.(2:n1))+sum(log.(2:n2))+sum(log.(2:n3))+sum(log.(2:n4))))
